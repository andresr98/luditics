...
export class SeguimientosPage {
  type: string;
  student: Student;
  behavioralDatas: Array<Student> = [];
  cognitiveDatas: Array<Student> = [];
  followUpData: Student;
  formatDate: string;
  typeCategory = {
    behavioral: 1,
    cognitive: 2
  ...
  constructor(
    ...
    this.images = [
      "assets/icon/Aislamiento.png",
      "assets/icon/Frustracion.png",
      "assets/icon/Impulsividad.png",
      "assets/icon/Solidaridad.png",
      "assets/icon/Armonioso.png"
    ];
  }
  ...
  getBehavioralFollowUP(student: Student) {
    ...
    this.studentProvider
      .getBehavioralFollowUP(
        this.student.id,
        this.typeCategory.behavioral,
        this.formatDate
      )
      .subscribe(
        data => {
  ...
  insertCategoryData() {
    ...
    this.loading = this.loadingCtrl.create({
      content: "Insertando datos a la base de datos..."
    });
    this.loading.present();
    this.studentProvider.insertCategoryData(2, this.formatDate).subscribe(
      data => {
        if (data.status == 201) {
          this.loading.dismissAll();
          this.showMessage(
            "El seguimiento para el día " +
              date[0] +
              " - " +
              date[1] +
              " - " +
      ...
      error => {
        this.loading.dismissAll();
        this.showMessage(
          "El seguimiento dirario no se puede generar. Verifique su conexión",
          true
        );
      }
    );
        ...
        if (element.changed) {
          this.studentProvider
            ...
            .subscribe(
              ...
              error => {
                this.showMessage(
                  "Verifique su conexión a internet. No se puede actualizar la información comportamental de " +
                    element.categoria__nombre,
                  true
                );
              }
            );
        ...
        if (element.changed) {
          this.studentProvider
            ...
            .subscribe(
              ...
              error => {
                this.showMessage(
                  "Verifique su conexión a internet. No se puede actualizar la información cognitiva de " +
                    element.categoria__nombre,
                  true
                );
              }
